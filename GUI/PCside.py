...
ser = serial.Serial('COMX', 9600)  # Change 'COMX' to your ESP32's UART port

speed_scale = 1
limit_angle_scale = 180


def set_speed_scale(new_speed):
    speed_scale = new_speed
    # Send command to ESP32 to set new degree limit
    ser.write("set_speed_scale:{}\n".format(new_speed).encode())


def set_degree_limit(new_limit):
    limit_angle_scale = new_limit
    # Send command to ESP32 to set new degree limit
    ser.write("set_degree_limit:{}\n".format(new_limit).encode())
...
class RadarControlApp:
    def __init__(self, root):
        ...
        self.limit_angle_label = tk.Label(root, text="Angle Restriction:")
        self.limit_angle_scale = tk.Scale(root, from_=0, to=180, orient="horizontal")
        ...
        self.limit_angle_label.grid(row=1, column=0, padx=10, pady=10)
        self.limit_angle_scale.grid(row=1, column=1, padx=10, pady=10)
    ...
    def update_radar_display(self):
        if limit_angle_scale != self.limit_angle_scale.get():
            set_degree_limit(self.limit_angle_scale.get())
        if speed_scale != self.speed_scale.get():
            set_speed_scale(self.speed_scale.get())
        if self.scanning:
            # Simulate radar scan motion (single sweep line)
            if ser.in_waiting > 0:
                self.current_angle = ser.readline().decode().strip()
...
while True:
        ...
        # Example: You can add your logic to handle received data here
