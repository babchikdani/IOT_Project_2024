...
#define SERVO_MIN_PWN 544
#define SERVO_MAX_PWN 2400
#define STOP 0
#define START 1
#define BAD_READ (-1)
...
uint8_t user_input_speed, user_input_max_angle, user_input_min_angle, user_input_max_dist, user_input_min_dist;
uint8_t start_stop, sweep_delay; 
...
void setup() {
  ...
  ESP32PWM::allocateTimer(0);
	...
	myservo.attach(servoPin, SERVO_MIN_PWN, SERVO_MAX_PWN); // attaches the servo on pin 18 to the servo object
...
void loop() {
  ...
  object_min_dist = 1000000;   // "Inifinity"
  uint8_t angle_at_min_dist = 0;
  // sweep right
	for (uint8_t pos = user_input_min_angle; pos <= user_input_max_angle; pos += 1) { // goes from 0 degrees to 180 degrees
  ...
  // sweep left
	for (uint8_t pos = user_input_max_angle; pos >= user_input_min_angle; pos -= 1) { // goes from 180 degrees to 0 degrees
		myservo.write(pos);    // tell servo to go to position in variable 'pos'
		delay(sweep_delay);    
    read_dist = readPrintDistanceAngle(pos);
    if(read_dist < object_min_dist && read_dist >= user_input_min_dist && read_dist <= user_input_max_dist && read_dist != BAD_READ){
...
inline void turnToMinAngle(uint8_t min_angle, uint16_t min_dist){
...
inline uint16_t readPrintDistanceAngle(uint8_t angle){
  uint16_t distance = 0;
