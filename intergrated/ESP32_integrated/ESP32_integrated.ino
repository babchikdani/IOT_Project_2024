...
void setup() {
  ...
  servo_init();
  // rotate180_check(); // check the mechanical state
  pinMode(LED_BUILTIN, OUTPUT);
}

void loop() { 
  // initialize 
  uint8_t metrics[6] = {1, 0, 180, 50, 700, 1};
  // guiSerial.flush();
  ...
  int angle_at_min_dist;
  // sweeps right
	for (int pos = user_input_min_angle; pos <= user_input_max_angle; pos += 1) { // goes from 0 degrees to 180 degrees
		// in steps of 1 degree
		myservo.write(pos);    // tell servo to go to position in variable 'pos'
		delay(sweep_delay);    // waits at least15ms for the servo to reach the position
    read_dist = readPrintDistanceAngle(pos);
    if(read_dist < object_min_dist && read_dist >= user_input_min_dist && read_dist <= user_input_max_dist){
  ...
  // sweeps left
	for (int pos = user_input_max_angle; pos >= user_input_min_angle; pos -= 1) { // goes from 180 degrees to 0 degrees
		myservo.write(pos);    // tell servo to go to position in variable 'pos'
		delay(sweep_delay);    // waits at least 15ms for the servo to reach the position
    read_dist = readPrintDistanceAngle(pos);
    if(read_dist < object_min_dist && read_dist >= user_input_min_dist && read_dist <= user_input_max_dist){
      angle_at_min_dist = pos;
      object_min_dist = read_dist;
...
inline void turnToMinAngle(int min_angle, int min_dist){
  ...
  delay(15);  // waits 15 ms for servo to reach its angle
  Serial.print("\n-------------------------------------------\n");
...
// checks the mechanic state of the engine
inline void rotate180_check(){
